...
系统 SHALL 在页脚左侧提供一条连续、无间隙的 10 等宽槽位导航带，并按固定顺序承载首批迁移入口：主监控、合同数据编辑、参数设定、格式设定、管捆编辑；未纳入本次迁移范围的槽位不得被映射到新的业务页面。

#### Scenario: 按固定顺序呈现已迁移入口

- **GIVEN** 当前页面挂载在 `HomePage` 壳层下
- **WHEN** 系统渲染页脚左侧导航带
- **THEN** 前 5 个槽位必须按“主监控、合同数据编辑、参数设定、格式设定、管捆编辑”的顺序显示可交互入口

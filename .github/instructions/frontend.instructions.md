...
- Do not introduce frontend-only copies of shared domain types. Add or adjust shared interfaces in the relevant `packages/shared/src/*.ts` module first, then keep `packages/shared/src/index.ts` exporting them.
...
- `frontend/src/stores/realtimeData.ts` is the canonical realtime store. It currently manages `tag1`, `tag2`, `tag3`, and `PlanInfo`; extend `packages/shared/src/redis_types.ts` and this store together when adding new realtime tags.
